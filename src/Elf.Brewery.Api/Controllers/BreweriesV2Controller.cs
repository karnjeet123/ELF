...
public sealed class BreweriesV2Controller
    : ControllerBase
{
    private readonly IBreweryService service;
    private readonly ILogger<BreweriesV2Controller> logger;
    ...
    {
        logger.LogInformation("List breweries (in-memory) search={Search} sortBy={SortBy} page={Page}",
    ...
    {
        logger.LogInformation("Refreshing in-memory breweries from external provider");
